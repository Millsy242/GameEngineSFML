...
public:
    ...
    void Move(sf::Vector2f amount, float dt); 
 ...
 int main()
 {
     ...
     while(window.isOpen()) // As long as the window is open
     {
         ...
         while(window.pollEvent(event))
         {
             ...
             {
                 standard.zoom(1.f+event.mouseWheel.delta*0.16f);
                 left.zoom(1.f+event.mouseWheel.delta*0.16f);
                 right.zoom(1.f+event.mouseWheel.delta*0.16f);
                 moved = true;
             }
